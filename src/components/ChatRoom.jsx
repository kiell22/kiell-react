...
export default function ChatRoom() {
  ...
  const [isChatVisible, setIsChatVisible] = useState(true); // Tambahkan state ini
  ...
  // Fungsi reset chatroom
  const resetChat = async () => {
    if (!window.confirm("Yakin ingin menghapus semua pesan dan menyembunyikan chatroom?")) return;
    const q = await getDocs(collection(db, "messages"));
    const batchDeletes = q.docs.map((d) => deleteDoc(doc(db, "messages", d.id)));
    await Promise.all(batchDeletes);
    setIsChatVisible(false); // Sembunyikan chatroom setelah reset
  };

  // Untuk menampilkan chatroom lagi (opsional)
  const showChatRoom = () => setIsChatVisible(true);

  // Jika chatroom disembunyikan, tampilkan pesan saja
  if (!isChatVisible) {
    return (
      <div className="flex flex-col items-center justify-center mt-24">
        <p className="text-gray-400 text-lg mb-4">Chatroom disembunyikan.</p>
        <button
